...
GMCS_FLAGS="-target:exe -r:./lib/SwinGame.dll,./lib/GameFramework.dll" #" -r:Microsoft.VisualBasic"
CS_FLAGS="-optimize+"
...
doCompileGameFramework()
{
	mcs `find ./GameFramework -name "*.cs"` -r:./lib/SwinGame.dll -t:library -out:./lib/GameFramework.dll
}
...
doCompile()
{
    ...
    if [ "$OS" = "$WIN" ]; then
        "${GMCS_BIN}" ${GMCS_FLAGS} ${CS_FLAGS} -out:"${OUT_DIR}/${GAME_NAME}.exe" `find ${SRC_DIR} -name "*.cs"` >> ${LOG_FILE}
    else
        "${GMCS_BIN}" ${GMCS_FLAGS} ${CS_FLAGS} -out:"${OUT_DIR}/${GAME_NAME}.exe" `find ${SRC_DIR} -name "*.cs"` >> ${LOG_FILE}
    fi
...
doLinuxPackage()
{
    ...
    cp -R -p "./lib/GameFramework.dll" "${OUT_DIR}/"
